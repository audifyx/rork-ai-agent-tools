...
const NUM_PAGES = 2;
const STORAGE_KEY = "@openclaw_home_v4";
...
// ── Single App Icon ───────────────────────────────────────────────
function AppIcon({
  app, wiggling, selected, onPress, onLongPress, badge,
}: {
  ...
  return (
    <View style={{ width: CELL_W, height: CELL_H, alignItems: "center", paddingTop: 10 }}>
      <TouchableOpacity
        ...
        activeOpacity={1} delayLongPress={420}
      >
        <Animated.View style={{ alignItems: "center", transform: [{ scale }, { rotate: spin }] }}>
          ...
          {selected && (
            <View style={[st.selRing, { width: ICON_SIZE + 6, height: ICON_SIZE + 6, borderRadius: ICON_R + 3 }]} />
          )}
          <View style={[st.iconBox, { width: ICON_SIZE, height: ICON_SIZE, borderRadius: ICON_R, backgroundColor: app.color }]}>
            ...
            {isSoon && (
              <View style={st.soonLay}>
                <Text style={st.soonTxt}>SOON</Text>
              </View>
            )}
          </View>
...
// ── Folder Icon ───────────────────────────────────────────────────
function FolderIcon({
  folder, apps, wiggling, onPress, onLongPress,
}: {
...
function FolderModal({
  folder, apps, visible, onClose, onOpen,
}: {
    ...
    <View style={st.root}>
      ...
      <View style={[st.dots, { bottom: insets.bottom + 98 }]}>
        {Array.from({ length: NUM_PAGES }, (_, i) => (
          <TouchableOpacity
            key={i}
            onPress={() => scrollRef.current?.scrollTo({ x: W * i, animated: true })}
          >
