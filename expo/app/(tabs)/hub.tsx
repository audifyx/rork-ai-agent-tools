...
import {
  Alert, Animated, Dimensions, Linking, Modal,
  PanResponder, ScrollView, StatusBar, StyleSheet,
...
const NUM_PAGES = 2;
const STORAGE_KEY = "@openclaw_home_v3";
    ...
    <View style={{ width: CELL_W, height: CELL_H, alignItems: "center", paddingTop: 10 }}>
      <TouchableOpacity onPress={onPress} onLongPress={onLongPress} onPressIn={pressIn} onPressOut={pressOut} activeOpacity={1} delayLongPress={420}>
        <Animated.View style={{ alignItems: "center", transform: [{ scale }, { rotate: spin }] }}>
          {/* Delete dot */}
          {wiggling && (
            <View style={st.delDot}>
              <Text style={st.delDotTxt}>✕</Text>
            </View>
          )}
          {/* Selection ring */}
...
export default function HomeScreen() {
  ...
  const deleteApp = (id: string) => {
    // Just move to a "hidden" state — can't truly delete built-in apps
  ...
  return (
    <View style={st.root}>
      ...
      <View style={st.glow1} /><View style={st.glow2} />
      ...
      <View style={[st.top, { paddingTop: insets.top + 6 }]}>
        {showSearch ? (
          <View style={st.searchBox}>
            ...
            <TextInput style={st.searchIn} placeholder="Search apps..." placeholderTextColor={Colors.textMuted} value={search} onChangeText={setSearch} autoFocus />
            <TouchableOpacity onPress={() => { setShowSearch(false); setSearch(""); }}><X size={15} color={Colors.textMuted} /></TouchableOpacity>
          </View>
        ) : (
          <>
            <View>
            ...
            <View style={{ flexDirection: "row", gap: 8 }}>
              <TouchableOpacity style={st.topBtn} onPress={() => setShowSearch(true)}><Search size={16} color={Colors.text} /></TouchableOpacity>
              <TouchableOpacity style={st.topBtn} onPress={() => Alert.alert("Sign Out?", "", [{ text: "Cancel", style: "cancel" }, { text: "Sign Out", style: "destructive", onPress: signOut }])}><LogOut size={16} color={Colors.textMuted} /></TouchableOpacity>
            </View>
          </>
        )}
      </View>
        ...
        <View style={[st.searchDrop, { top: insets.top + 68 }]}>
          {searchResults.length === 0
            ...
            : searchResults.map(app => (
              <TouchableOpacity key={app.id} style={st.searchRow} onPress={() => { setShowSearch(false); setSearch(""); handleApp(app); }}>
                <View style={[st.searchIco, { backgroundColor: app.color, borderRadius: 10 }]}><Text style={{ fontSize: 18 }}>{app.emoji}</Text></View>
      ...
      {wiggling && (
        <View style={[st.wiggleBar, { top: insets.top + 68 }]}>
          <TouchableOpacity style={st.wBtn} onPress={exitWiggle}><Text style={st.wBtnTxt}>Done</Text></TouchableOpacity>
          {selected.length >= 2 && (
            <TouchableOpacity style={[st.wBtn, { backgroundColor: Colors.accent, borderColor: Colors.accent }]} onPress={() => setShowNewFolder(true)}>
              <Text style={[st.wBtnTxt, { color: "#fff" }]}>📁 Folder ({selected.length})</Text>
            </TouchableOpacity>
          )}
          {selected.length < 2 && (
            <Text style={st.wTip}>{selected.length === 0 ? "Tap apps to select · Long-press to delete" : "Select 1 more to create a folder"}</Text>
          )}
        </View>
      )}
            ...
            <View style={st.grid}>
              {pageFolders(pi).map(folder => (
                <FolderIcon
                  ...
                  apps={apps}
                  wiggling={wiggling}
                  onPress={() => { if (!wiggling) { setActiveFolderModal(folder); setFolderModalVisible(true); } }}
              ...
              {pageApps(pi).map(app => (
                <AppIcon
                  ...
                  onLongPress={() => {
                    if (!wiggling) { enterWiggle(); }
                    else {
                      Alert.alert(`Remove "${app.name}"?`, "It will be hidden from your home screen.", [
                        { text: "Cancel", style: "cancel" },
                        { text: "Remove", style: "destructive", onPress: () => deleteApp(app.id) },
                      ]);
                    }
                  }}
                  badge={badges[app.id]}
      ...
      {!wiggling && (
        <View style={[st.dock, { bottom: insets.bottom + 12 }]}>
          {[
            ...
            <TouchableOpacity key={d.id} style={st.dockItem} onPress={() => router.push(d.route as any)} onLongPress={enterWiggle}>
              <View style={[st.dockIco, { backgroundColor: d.color }]}>
                <Text style={{ fontSize: 22 }}>{d.emoji}</Text>
                {d.id === "notifs" && (badges.notifs ?? 0) > 0 && (
                  <View style={st.dockBadge}><Text style={st.dockBadgeTxt}>{badges.notifs}</Text></View>
                )}
              </View>
      ...
      <Modal visible={showNewFolder} transparent animationType="fade" onRequestClose={() => setShowNewFolder(false)}>
        <View style={st.mBg}>
          <View style={st.mBox}>
            ...
            <TextInput style={st.mInput} value={folderName} onChangeText={setFolderName} placeholder="Folder name" placeholderTextColor={Colors.textMuted} selectTextOnFocus autoFocus />
...
const st = StyleSheet.create({
  ...
  folderBox: { backgroundColor: "rgba(255,255,255,0.14)", alignItems: "center", justifyContent: "center", overflow: "hidden",
  ...
  dockIco: { width: 50, height: 50, borderRadius: 15, alignItems: "center", justifyContent: "center", position: "relative",
